...
<body class="login">
<div id="login"><h1><a href="<?php echo esc_url( home_url( '/' ) ); ?>" title="<?php _e('Powered by BA'); ?>"><?php bloginfo('name'); ?></a></h1> <?php
...
if ( ! $allow )
    ...
    $rest_url = network_site_url("wp-login.php?action=rp&key=$key&login=" . rawurlencode($user_login), 'login');
    ...
    $message .= '<a href="' . $rest_url . "\">$rest_url</a><br/><br/>\r\n";
    $message .= '</body></html>';
...
function ba_get_redirect_url() {
    return apply_filters( 'registration_redirect', !empty( $_REQUEST['redirect_to'] ) ? '&redirect_to=' . urlencode($_REQUEST['redirect_to']) : '' );
}
...
<a href="<?php echo esc_url( wp_login_url() ); ?>"><?php _e( 'Log in' ); ?></a>
<?php if ( get_option( 'users_can_register' ) ) : ?>
 | <a href="<?php echo esc_url( site_url( 'wp-login.php?action=register' . ba_get_redirect_url(), 'login' ) ); ?>"><?php _e( 'Register' ); ?></a>
<?php endif; ?>
</p>
...
case 'register' :
    ...
    if ( $http_post ) {
        ...
        if ( !is_wp_error($errors) ) {
            $redirect_to = 'wp-login.php?' . ba_get_redirect_url(); //!empty( $_POST['redirect_to'] ) ? $_POST['redirect_to'] : ''; /*'wp-login.php?checkemail=registered'*/;
            wp_safe_redirect( $redirect_to );
            exit();
        }
    }
