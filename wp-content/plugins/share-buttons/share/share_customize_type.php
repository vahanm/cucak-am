<?php
	...
	if($customize_type=='original_count') {
		...
		$shares_json = file_get_contents('http://' . 'graph.facebook.com/?id=' . $url, false, $context);
	
		$facebookSharesCount = 0;
	
		if(isset($shares_json) && strlen($shares_json) > 10) {
			$agent = json_decode($shares_json);
		if(isset($agent) && isset($agent->shares)) {
				$facebookSharesCount = $agent->shares;
			}
		}

		$facebook .= "<div class='facebook-classic'>\r\n";
		if ($facebookSharesCount > 0) {
			$facebook .= "<div class='facebook-classic-shares-count'>$facebookSharesCount</div>";
		}
		$facebook .= "<noindex><a rel='nofollow' title='".__('Share to Facebook', $this->plugin_domain)."' href=\"#facebook\" name=\"facebook\" onclick=\"new_window('http://www.facebook.com/sharer.php?u=$url');\">";
