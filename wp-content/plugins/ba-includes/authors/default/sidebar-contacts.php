...
$pcont .= '<div class="sidebar-post-info" freez="fixed-sidebar-post-info" style="text-align: left;">';
...
if ($val_userid > 0 && $val_userid != 2) {
    ...
    $val_address = trim(get_user_meta($val_userid, 'address', true));
    $val_aname = trim(get_user_meta($val_userid, 'display_name', true));
    $val_phone = trim(get_user_meta($val_userid, 'phone', true));
    $val_aemail = trim(get_user_meta($val_userid, 'email', true));
    ...
    $val_user_url = trim(get_user_meta($val_userid, 'user_url', true));
    ...
    if ($val_aname) {
        ...
        if ($val_userid == 2)
            ...
            $pcont .= '<td><strong><a href="' . site_url("/$val_userid") . '"title="' . __('view all posts by this user') . '">' . $val_aname . '</a></strong></td></tr>';
    }
    ...
    if (isset($val_user_url) && strlen($val_user_url) > 3) {
        ...
        $pcont .= '<tr><td title="' . __('Web page') . '">' . contact_icon($iconName) . '</td><td><strong><aCRON_BR href="' . $link . '">' . $linkName . '</a></strong></td></tr>';
    }
