...
render_group_sub(__('House information'));

render_location('item_location', __('House location'));

render_value('address', __('Address'), '%s ' );

render_value('structureyear', __('Year of the structure'), __('%sy'), array(
	1940 => __('%sy or older')
	));

render_value('floorquantity', __('Floor quantity'), __('%s storeys'), array(
	1 => __('%s storey')
	));
...
//render_yes_no("summercottage", __('Summer cottage'));
...
render_value('container', __('Water container'), __('%s litres'), array( 
	0 => 'image:no'
	));
...
render_value('roomquantityhouse', __('Room quantity'), __('%s rooms'), array(
	1 => __('%s room')
	));
