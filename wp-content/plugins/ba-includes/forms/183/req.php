...
function get_form_errors()
{
	...
	$error .= require_oneof(
		require_length_min('opticdevice', 2, __('Model is invalid.')),
		require_length_max('opticdevice', 30, _t('Model must be contents %s chars maximum.',30))
		);

	$error .= require_selection('optic', __('Please select the device type.'));
