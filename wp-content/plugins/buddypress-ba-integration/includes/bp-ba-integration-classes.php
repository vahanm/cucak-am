 ...
 * BP_BA_Intergration_Highfive object in get()) is one suggested implementation.
 */

class BP_BA_Intergration_Highfive {
	var $id;
	var $high_fiver_id;
	var $recipient_id;
	var $date;
	...
	function populate() {
		...
		if ( $row = $wpdb->get_row( $wpdb->prepare( "SELECT * FROM {$bp->ba_integration->table_name} WHERE id = %d", $this->id ) ) ) {
	...
	function save() {
		 ...
		 * filter to 'bp-ba-integration-filters.php'
		 *
		 *   add_filter( 'example_data_fieldname1_before_save', 'wp_filter_kses' );
		 */

		$this->high_fiver_id = apply_filters( 'bp_ba_integration_data_high_fiver_id_before_save', $this->high_fiver_id, $this->id );
		$this->recipient_id  = apply_filters( 'bp_ba_integration_data_recipient_id_before_save', $this->recipient_id, $this->id );
		$this->date	     = apply_filters( 'bp_ba_integration_data_date_before_save', $this->date, $this->id );

		// Call a before save action here
		do_action( 'bp_ba_integration_data_before_save', $this );
		...
		if ( $this->id ) {
			...
			$wp_update_post_args = array(
				...
				'post_title'	=> sprintf( __( '%1$s high-fives %2$s', 'bp-ba-integration' ), bp_core_get_user_displayname( $this->high_fiver_id ), bp_core_get_user_displayname( $this->recipient_id ) )
			);
			...
			if ( $result ) {
				update_post_meta( $result, 'bp_ba_integration_recipient_id', $this->recipient_id );
			}
		} else {
			// Set up the arguments for wp_insert_post()
			$wp_insert_post_args = array(
				'post_status'	=> 'publish',
				'post_type'	=> 'ba_integration',
				'post_author'	=> $this->high_fiver_id,
				'post_title'	=> sprintf( __( '%1$s high-fives %2$s', 'bp-ba-integration' ), bp_core_get_user_displayname( $this->high_fiver_id ), bp_core_get_user_displayname( $this->recipient_id ) )
			);
		...
		do_action( 'bp_ba_integration_data_after_save', $this );
		...
		if ( empty( $this->query ) ) {
			...
			if ( $recipient_id ) {
				$query_args['meta_query'][] = array(
					'key'	  => 'bp_ba_integration_recipient_id',
	...
	/**
	 * Part of our bp_ba_integration_has_high_fives() loop
	 *
	 * @package BuddyPress_Skeleton_Component
	 * @since 1.6
	 */
