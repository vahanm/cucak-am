...
/**
 * bp_ba_integration_load_template_filter()
 *
 ...
 *
 * This will become clearer in the function bp_ba_integration_screen_one() when you want to load
 * a template file.
 */
function bp_ba_integration_load_template_filter( $found_template, $templates ) {
	global $bp;
	...
	return apply_filters( 'bp_ba_integration_load_template_filter', $found_template );
}
add_filter( 'bp_located_template', 'bp_ba_integration_load_template_filter', 10, 2 );
 ...
 * 	or functions in your bp-ba-integration-classes.php file to fetch what you need. Spraying database
...
function bp_ba_integration_accept_terms() {
	 ...
	 */
	check_admin_referer( 'bp_ba_integration_accept_terms' );
	...
	bp_ba_integration_record_activity( array(
		'type' => 'accepted_terms',
		'action' => apply_filters( 'bp_ba_integration_accepted_terms_activity_action', sprintf( __( '%s accepted the really exciting terms and conditions!', 'bp-ba-integration' ), $user_link ), $user_link ),
	) );

	/* See bp_ba_integration_reject_terms() for an explanation of deleting activity items */
	...
	do_action( 'bp_ba_integration_accept_terms', $bp->loggedin_user->id );
...
/**
 * bp_ba_integration_reject_terms()
 *
 * Rejects the terms and conditions screen for the logged in user.
 * Records an activity stream item for the user.
 */
function bp_ba_integration_reject_terms() {
	global $bp;

	check_admin_referer( 'bp_ba_integration_reject_terms' );
	...
	bp_ba_integration_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_ba_integration_rejected_terms_activity_action', sprintf( __( '%s rejected the really exciting terms and conditions.', 'bp-ba-integration' ), $user_link ), $user_link ),
	) );
	...
	do_action( 'bp_ba_integration_reject_terms', $bp->loggedin_user->id );

	return true;
}

/**
 * bp_ba_integration_send_high_five()
 *
 ...
 */
function bp_ba_integration_send_highfive( $to_user_id, $from_user_id ) {
	global $bp;

	check_admin_referer( 'bp_ba_integration_send_high_five' );
	 ...
	 * bp-ba-integration-classes.php that would run the SQL query.
	 */
	...
	if ( !in_array( $from_user_id, (array)$existing_fives ) ) {
		...
		$high_five = new BP_BA_Intergration_Highfive( $db_args );
		$high_five->save();
	}
	 ...
	 * this notification in bp_ba_integration_format_notifications() using the 'new_high_five' action.
	 */
	bp_core_add_notification( $from_user_id, $to_user_id, $bp->ba_integration->slug, 'new_high_five' );
	...
	bp_ba_integration_record_activity( array(
		'type' => 'rejected_terms',
		'action' => apply_filters( 'bp_ba_integration_new_high_five_activity_action', sprintf( __( '%s high-fived %s!', 'bp-ba-integration' ), $from_user_link, $to_user_link ), $from_user_link, $to_user_link ),
		'item_id' => $to_user_id,
	) );

	/* We'll use this do_action call to send the email notification. See bp-ba-integration-notifications.php */
	do_action( 'bp_ba_integration_send_high_five', $to_user_id, $from_user_id );

	return true;
}

/**
 * bp_ba_integration_get_highfives_for_user()
 *
 * Returns an array of user ID's for users who have high fived the user passed to the function.
 */
function bp_ba_integration_get_highfives_for_user( $user_id ) {
	global $bp;
...
function bp_ba_integration_remove_data( $user_id ) {
	...
	delete_user_meta( $user_id, 'bp_ba_integration_some_setting' );

	do_action( 'bp_ba_integration_remove_data', $user_id );
}
add_action( 'wpmu_delete_user', 'bp_ba_integration_remove_data', 1 );
add_action( 'delete_user', 'bp_ba_integration_remove_data', 1 );
