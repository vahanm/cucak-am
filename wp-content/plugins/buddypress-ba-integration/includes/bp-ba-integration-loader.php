 ...
 *
 *
 *	if ( !defined( 'BP_BA_INTEGRATION_CONSTANT' ) )
 *		define ( 'BP_BA_INTEGRATION_CONSTANT', 'some value' // or some value without quotes if integer );
 */
 ...
 * Throughout this example the text domain used is 'bp-ba-integration', you can use whatever you want.
 * Put the text domain as the second parameter:
 *
 * __( 'This text will be translatable', 'bp-ba-integration' ); // Returns the first parameter value
 * _e( 'This text will be translatable', 'bp-ba-integration' ); // Echos the first parameter value
 */

if ( file_exists( dirname( __FILE__ ) . '/languages/' . get_locale() . '.mo' ) )
	load_textdomain( 'bp-ba-integration', dirname( __FILE__ ) . '/bp-ba-integration/languages/' . get_locale() . '.mo' );
 ...
 * @since 1.6
 */
class BP_BA_Intergration_Component extends BP_Component {
	 ...
	 *		 to see how BP_BA_INTEGRATION_PLUGIN_DIR was defined.
	 *
	 * @package BuddyPress_Skeleton_Component
	 * @since 1.6
	 */
	function __construct() {
		...
		parent::start(
			'ba_integration',
			__( 'BA Integration', 'bp-ba-integration' ),
			BP_BA_INTEGRATION_PLUGIN_DIR
		);
	 ...
	 *       setup):
	 *           /wp-content/plugins/bp-ba-integration/notifications.php
	 *           /wp-content/plugins/bp-ba-integration/filters.php
	 ...
	 *           /wp-content/plugins/bp-ba-integration/bp-ba-integration/notifications.php
	 *           /wp-content/plugins/bp-ba-integration/bp-ba-integration/filters.php
	 ...
	 *           /wp-content/plugins/bp-ba-integration/bp-ba-integration/bp-ba-integration-notifications.php
	 *           /wp-content/plugins/bp-ba-integration/bp-ba-integration/bp-ba-integration-filters.php
	 *
	 ...
	 *			    bp_ba_integration_has_items()), those functions should go in this file.
	...
	function includes() {
		...
		// Files to include
		$includes = array(
			'includes/bp-ba-integration-actions.php',
			'includes/bp-ba-integration-screens.php',
			'includes/bp-ba-integration-filters.php',
			'includes/bp-ba-integration-classes.php',
			'includes/bp-ba-integration-activity.php',
			'includes/bp-ba-integration-template.php',
			'includes/bp-ba-integration-functions.php',
			'includes/bp-ba-integration-notifications.php',
			'includes/bp-ba-integration-widgets.php',
			'includes/bp-ba-integration-cssjs.php',
			'includes/bp-ba-integration-ajax.php'
		);
		...
		if ( is_admin() || is_network_admin() ) {
			include( BP_BA_INTEGRATION_PLUGIN_DIR . '/includes/bp-ba-integration-admin.php' );
		}
	}
	 ...
	 *				  'ba_integration' portion of the URL is formed by the 'slug'.
	 *				  Site admins can customize this value by defining
	 *				  BP_BA_INTEGRATION_SLUG in their wp-config.php or bp-custom.php
	 *				  files.
	 ...
	 *				    http://testbp.com/ba-integration/test
	 *				  'ba_integration' is a root slug. This should always be defined
	...
	function setup_globals() {
		...
		if ( !defined( 'BP_BA_INTEGRATION_SLUG' ) )
			define( 'BP_BA_INTEGRATION_SLUG', $this->id );
		...
		$global_tables = array(
			'table_name'      => $bp->table_prefix . 'bp_ba_integration'
		);

		// Set up the $globals array to be passed along to parent::setup_globals()
		$globals = array(
			'slug'                  => BP_BA_INTEGRATION_SLUG,
			'root_slug'             => isset( $bp->pages->{$this->id}->slug ) ? $bp->pages->{$this->id}->slug : BP_BA_INTEGRATION_SLUG,
			'has_directory'         => true, // Set to false if not required
			'notification_callback' => 'bp_ba_integration_format_notifications',
	 ...
	 * BP_BA_Intergration_Component::setup_admin_bar().
	 *
	 * @global obj $bp
	 */
	function setup_nav() {
		...
		$main_nav = array(
			'name' 		      => __( 'Example', 'bp-ba-integration' ),
			'slug' 		      => bp_get_ba_integration_slug(),
			'position' 	      => 80,
			'screen_function'     => 'bp_ba_integration_screen_one',
			'default_subnav_slug' => 'screen-one'
		);

		$ba_integration_link = trailingslashit( bp_loggedin_user_domain() . bp_get_ba_integration_slug() );

		// Add a few subnav items under the main Example tab
		$sub_nav[] = array(
			'name'            =>  __( 'Screen One', 'bp-ba-integration' ),
			'slug'            => 'screen-one',
			'parent_url'      => $ba_integration_link,
			'parent_slug'     => bp_get_ba_integration_slug(),
			'screen_function' => 'bp_ba_integration_screen_one',
			'position'        => 10
		);
		...
		$sub_nav[] = array(
			'name'            =>  __( 'Screen Two', 'bp-ba-integration' ),
			'slug'            => 'screen-two',
			'parent_url'      => $ba_integration_link,
			'parent_slug'     => bp_get_ba_integration_slug(),
			'screen_function' => 'bp_ba_integration_screen_two',
			'position'        => 20
		);
		...
		// it like this. See bp_ba_integration_screen_settings_menu() for more info
		bp_core_new_subnav_item( array(
			'name' 		  => __( 'Example', 'bp-ba-integration' ),
			'slug' 		  => 'ba-integration-admin',
			'parent_slug'     => bp_get_settings_slug(),
			'parent_url' 	  => trailingslashit( bp_loggedin_user_domain() . bp_get_settings_slug() ),
			'screen_function' => 'bp_ba_integration_screen_settings_menu',
			'position' 	  => 40,
	 ...
	 * hook to init. See BP_BA_Intergration_Component::__construct().
	 *
	 * @package BuddyPress_Skeleton_Component
	 * @since 1.6
	...
	function register_post_types() {
		...
		$labels = array(
			'name'	   => __( 'High Fives', 'bp-ba-integration' ),
			'singular' => __( 'High Five', 'bp-ba-integration' )
		);

		// Set up the argument array for register_post_type()
		$args = array(
			'label'	   => __( 'High Fives', 'bp-ba-integration' ),
			'labels'   => $labels,
			'public'   => false,
			'show_ui'  => true,
		...
		// Here we are using $this->id ('ba_integration') as the name of the post type. You may
 ...
 * BP_BA_Intergration_Component is loaded after BuddyPress's core components. This is a good thing because
 ...
 * routine. Using print_r( $bp->ba_integration ) or var_dump( $bp->ba_integration ) at the end of this function
 ...
 *   function bp_ba_integration_var_dump() {
 *   	  global $bp;
 *	  var_dump( $bp->ba_integration );
 *   }
 *   add_action( 'bp_init', 'bp_ba_integration_var_dump' );
 * It goes without saying that you should not do this on a production site!
 *
 * @package BuddyPress_Skeleton_Component
 * @since 1.6
 */
function bp_ba_integration_load_core_component() {
	global $bp;

	$bp->ba_integration = new BP_BA_Intergration_Component;
}
add_action( 'bp_loaded', 'bp_ba_integration_load_core_component' );
