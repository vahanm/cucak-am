		...
		if(!empty($csds_userRegAide_registrationFields)){
			foreach($csds_userRegAide_registrationFields as $fieldKey => $fieldName){
				...
				<?php 
					...
					<p>
					<input autocomplete="off" name="pass1" id="pass1" size="25" value="" type="password" tabindex="20" class="input" /></p></label></div>
					<div> <label><?php _e('Confirm Password:', 'csds_userRegAide');?> <p>
					<input autocomplete="off" name="pass2" id="pass2" size="25" value="" type="password" tabindex="20" class="input" /></p></label></div>
					<p>
...
function csds_new_user_notification($user_id, $plaintext_pass = '') {
	...
	$user_email = stripslashes($user->user_email);
		...
		@wp_mail(get_option('admin_email'), sprintf(__('%s -  New User Registration Notification'), $blogname), $message);
