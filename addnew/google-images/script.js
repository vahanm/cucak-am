    ...
    if (response.error && response.error.code > 0) {
        ...
        if (response.searchInformation && response.searchInformation.totalResults > 0) {
            for (var i = 0; i < response.items.length; i++) {
                ...
                document.getElementById("content").innerHTML += '<div class="article" link="' + item.link + '"><h2 class="title">' + item.htmlTitle + '</h2><p class="description">' + item.htmlSnippet + '</p></article>';
            }
        } else {
